...
void readfiles(char *filename, int filetype, int argmonth)
{
	// opening text file
	...
	while (fgets(line, sizeof line, dict) != NULL) {
		...
		case 1: 
			...
			if (fileinfo[i].month != argmonth && fileinfo[i].month != -1) {
				// if not wanted month, don't continue storing (especially name)
				// and -index so if more data, it will be overwritten
				printf("point a, %i %i\n", fileinfo[i].month, argmonth); //test
				i--;
				break;
			}
			token = strtok(NULL, " ");
		...
		case 2: 
			// estimates file
			for (int j = 0; j < sizeof(fileinfo)/sizeof(fileinfo[0]); j++) {
				// matches crontab and estimates names to get and store runtime data
				if (strcmp(fileinfo[j].name, token) == 0) {
					token = strtok(NULL, " ");
					fileinfo[j].runtime = atoi(token);
					break;
				}
			}
...
{
	int argmonth = my_atoi(argv[1], 1);	
	readfiles(argv[2], 1, argmonth);
	readfiles(argv[3], 2, argmonth); // argmonth redundant here
